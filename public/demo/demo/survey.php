<?php
	...
	if(mysqli_num_rows($lds)>0){
		while($ld = $lds->fetch_assoc()){
			echo "<h2><strong>Learning domain:</strong> " . $ld["Name"] . "(" . $ld["Abbr"] . ")</h2>";
			...
			if(mysqli_num_rows($survey)>0){
				while($question = $survey->fetch_assoc()){
					echo("<p>" . $question["Text"] . " <a href='delete.php?qID=" . $question["Question_ID"] . "'><button class='btn btn-danger'>x</button></a>");
				}
			}
			echo "<br>";
		}
